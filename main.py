...
import os
import sys
sys.path.append('.\Mexico\Code\src')
...
from find_files import find_func_files
...
for row in pd.read_csv('D:\Mexico\Code\group_bold.csv')['bids_name']:
    for f in t:
        if row in f:
            func_files.append(f)
...
import pandas as pd
behavioral = pd.read_csv('D:\Mexico\Code\group_bold.csv')
...
C = load("../data/difumo_atlases/512/components.pt")
...
# for f in features:
#     item=[]
#     for roi_index in range(f.shape[0]):
#         row=[]
#         mean=np.sum(f[roi_index,:])/f.shape[1]
#         std=np.std(f[roi_index,:])
#         # skew=skew(f[roi_index,:])
#         # kurtosis=kurtosis(f[roi_index,:])
#
#         row.append(mean)
#         row.append(std)
#         # row.append(skew)
...
data_dir='D:/Mexico/Code/fmri_data_graph/'
# set output paths
